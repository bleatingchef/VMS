...
const Faqs = () => {
  ...
  return (
    <div className='text-black flex flex-col items-center p-6'>
      <h2 className='md:text-4xl text-2xl font-semibold text-pink-800 mb-4'>{faqsData.heading}</h2>
        ...
        <div className="space-y-3 text-black">
          {faqsData.questions.map((faq, index) => (
            <div key={index} className="collapse collapse-arrow rounded-lg ">
              ...
              <label htmlFor={`faq-accordion-${index}`} className="collapse-title text-xs md:text-sm cursor-pointer block p-4">
                {faq.question}
              </label>
              <div className="collapse-content text-xs md:text-sm text-pink-800 px-4 py-2 peer-checked:block hidden">
                {faq.answer}
              </div>
            </div>
          ))}
      ...
      <h1 className=' text-pink-800 max-w-full text-xs sm:text-lg transition-transform transform hover:scale-110 duration-300 ease-in-out p-12'>
